...
public class AddressHandle {
		...
		(new BukkitRunnable() {
			...
			public void run() {
				...
				try {
					...
					JSONObject response = (JSONObject) requestObject.get();
					...
					if (AddressHandle.badIP(String.valueOf(response.get("countryCode")).trim(),
							(boolean) response.get("hosting"), (boolean) response.get("proxy"), String.valueOf(response.get("isp")), (boolean) response.get("mobile"))) {
	...
	public static boolean badIP(String country, boolean hosting, boolean proxy, String provider, boolean mobile) {
